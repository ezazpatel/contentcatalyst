...
export function CSVUpload({ onUpload }: CSVUploadProps) {
        ...
        try {
          ...
          const processedData = nonEmptyRows.map((row: any, index: number) => {
            try {
              ...
              return {
                title: row.Title?.trim() || `Blog Post About ${row.Keywords}`,
  ...
  const downloadTemplate = () => {
    const template = "Keywords,Title,Scheduled Date,Scheduled Time,Affiliate Product Names,Affiliate Product URLs,Content Instructions,SEO Title,SEO Description,Internal Link Title,Internal Link URL,Internal Link Description\n" +
      "vancouver trip itinerary,Vancouver Trip Guide,2025-03-22,18:30,Vancouver: Flyover Experience|Vancouver: Seaplane Tour,https://example.com/flyover|https://example.com/seaplane,Write about top tourist destinations,,,,,,";
